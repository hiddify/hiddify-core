...
func (cch *OldCommandClientHandler) WriteGroups(message libbox.OutboundGroupIterator) {
	...
	groups := []*OutboundGroupOld{}
	for message.HasNext() {
		group := message.Next()
		items := group.GetItems()
		groupItems := []*OutboundGroupItemOld{}
		for items.HasNext() {
			item := items.Next()
			groupItems = append(groupItems,
				&OutboundGroupItemOld{
					Tag:          item.Tag,
					Type:         item.Type,
		...
		groups = append(groups, &OutboundGroupOld{Tag: group.Tag, Type: group.Type, Selected: group.Selected, Items: groupItems})
	}
...
type OutboundGroupOld struct {
	Tag      string                  `json:"tag"`
	Type     string                  `json:"type"`
	Selected string                  `json:"selected"`
	Items    []*OutboundGroupItemOld `json:"items"`
}

type OutboundGroupItemOld struct {
	Tag          string `json:"tag"`
	Type         string `json:"type"`
