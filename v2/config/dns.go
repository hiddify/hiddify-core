...
func addForceDirect(options *option.Options, opt *HiddifyOptions) ([]option.DefaultDNSRule, error) {
	...
	for _, outbound := range options.Outbounds {
		...
		if server, ok := outbound.Options.(option.ServerOptionsWrapper); ok {
			...
			if dialer, ok := outbound.Options.(option.DialerOptionsWrapper); ok {
	...
	if len(dnsMap) > 0 {
		...
		for detour := range unique_dns_detours {
			...
			if detour != OutboundDirectTag {
				...
				remote_dns, err := getDNSServerOptions(dns_detour, opt.RemoteDnsAddress, DNSDirectTag, detour)
				if err != nil {
					return nil, err
				}
