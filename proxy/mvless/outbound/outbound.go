...
import (
	...
	"github.com/GFW-knocker/Xray-core/proxy/mvless/encoding"
...
func (h *Handler) Process(ctx context.Context, link *transport.Link, dialer internet.Dialer) error {
	...
	account := request.User.Account.(*mvless.MemoryAccount)
	...
	if request.Command == protocol.RequestCommandUDP && (requestAddons.Flow == mvless.XRV || (h.cone && request.Port != 53 && request.Port != 443)) {
	...
	postRequest := func() error {
		...
		var err error
		if requestAddons.Flow == mvless.XRV {
			if tlsConn, ok := iConn.(*tls.Conn); ok {
